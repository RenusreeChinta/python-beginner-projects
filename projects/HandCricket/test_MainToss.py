...
class Test_MainToss:
    ...
    def test_toss_user_wins_with_heads(self, input, randint):
        assert toss() == 1

    @pytest.mark.regression
    @patch('main.input', return_value='2')
    @patch('random.randint', return_value=2)
    def test_toss_user_wins_with_tails(self, input, randint):
        assert toss() == 1

    @pytest.mark.regression
    @patch('main.input', return_value='1')
    @patch('random.randint', return_value=2)
    def test_toss_user_loses_with_heads(self, input, randint):
        assert toss() == 2

    @pytest.mark.regression
    @patch('main.input', return_value='2')
    @patch('random.randint', return_value=1)
    def test_toss_user_loses_with_tails(self, input, randint):
        assert toss() == 2
