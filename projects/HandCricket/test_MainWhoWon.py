...
class Test_MainWhoWon:
    ...
    def test_player2_wins(self, monkeypatch):
        ...
        monkeypatch.setattr('builtins.print', lambda x: x)
        result = who_won(3, 5)
        assert result == "Player 2 won"

    @pytest.mark.positive
    def test_game_draw(self, monkeypatch):
        monkeypatch.setattr('builtins.input', lambda _: '5')
        monkeypatch.setattr('builtins.print', lambda x: x)
        result = who_won(5, 5)
        assert result == "The match ended in a draw"

    @pytest.mark.negative
    def test_negative_scores(self, monkeypatch):
        monkeypatch.setattr('builtins.input', lambda _: '-5')
        monkeypatch.setattr('builtins.print', lambda x: x)
        result = who_won(-3, -5)
        assert result == "Player 1 won"
