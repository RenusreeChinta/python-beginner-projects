...
# ********RoostGPT********
import pytest
from hill_cipher.encryption import hill_cipher

class Test_EncryptionHillCipher:
    @pytest.mark.regression
    def test_hill_cipher_valid_output(self):
        text = 'abc'
        key_matrix = [1, 2, 3, 4, 5, 6, 7, 8, 9]
        result = hill_cipher(text, key_matrix)
        assert result == [8, 26, 44], "The output cipher text does not match the expected output"
    ...
    def test_hill_cipher_different_key_matrices(self):
        ...
        result1 = hill_cipher(text, key_matrix1)
        result2 = hill_cipher(text, key_matrix2)
        assert result1 != result2, "The cipher texts for different key matrices are the same"
