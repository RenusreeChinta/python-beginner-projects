...
class Test_BmiCalculatorMain:
    ...
    @pytest.mark.parametrize("height, weight, result", [(1.75, 70, "Your BMI is 22.86, you have a normal weight.")])
    def test_valid_height_weight(self, height, weight, result):
        with patch('builtins.input', side_effect=[str(height), str(weight)]), \
             patch('builtins.print') as mock_print:
            main()
        mock_print.assert_called_with(result)

    @pytest.mark.parametrize("height, weight", [(0, 70), (-1.75, 70)])
    def test_invalid_height(self, height, weight):
        with patch('builtins.input', side_effect=[str(height), str(weight)]), \
             patch('builtins.print') as mock_print:
            main()
        mock_print.assert_called_with("Invalid input. Height should be greater than 0.")

    @pytest.mark.parametrize("height, weight", [(1.75, -70)])
    def test_invalid_weight(self, height, weight):
        with patch('builtins.input', side_effect=[str(height), str(weight)]), \
             patch('builtins.print') as mock_print:
            main()
        mock_print.assert_called_with("Invalid input. Please enter numerical values for height and weight.")

    @pytest.mark.parametrize("height, weight", [("abc", 70), (1.75, "xyz")])
    def test_non_numeric_inputs(self, height, weight):
        with patch('builtins.input', side_effect=[str(height), str(weight)]), \
             patch('builtins.print') as mock_print:
            main()
        mock_print.assert_called_with("Invalid input. Please enter numerical values for height and weight.")

    @pytest.mark.parametrize("height, weight, result", [(1.75, 50, "Your BMI is 16.33, you are underweight."),
    ...
    def test_bmi_categories(self, height, weight, result):
        with patch('builtins.input', side_effect=[str(height), str(weight)]), \
             patch('builtins.print') as mock_print:
            main()
        mock_print.assert_called_with(result)
