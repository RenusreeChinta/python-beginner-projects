...
class Test_BmiCalculatorReferenceChart:
    ...
    def test_reference_chart_valid_file(self, tmpdir):
        p = tmpdir.mkdir("sub").join("bmi.csv")
        p.write("bmi,weight,status\n18.5,60,Normal\n25,80,Overweight")
        os.chdir(tmpdir)
        output = reference_chart()
        assert output == "Here You can take the reference chart \n" \
                         "+------+--------+-------------+\n" \
                         "| bmi  | weight | status      |\n" \
                         "+------+--------+-------------+\n" \
                         "| 18.5 | 60     | Normal      |\n" \
    ...
    def test_reference_chart_empty_file(self, tmpdir):
        p = tmpdir.mkdir("sub").join("bmi.csv")
        p.write("")
        os.chdir(tmpdir)
