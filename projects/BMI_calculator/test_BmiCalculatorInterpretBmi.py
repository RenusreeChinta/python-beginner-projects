...
class Test_BmiCalculatorInterpretBmi:
    ...
    def test_interpret_bmi_for_underweight(self):
        ...
        assert result == f"Your BMI is {bmi}, you are underweight.", "Failed to correctly interpret underweight BMI"

    def test_interpret_bmi_for_normal_weight(self):
        bmi = 22.0
        result = interpret_bmi(bmi)
        assert result == f"Your BMI is {bmi}, you have a normal weight.", "Failed to correctly interpret normal weight BMI"

    def test_interpret_bmi_for_overweight(self):
        bmi = 28.0
        result = interpret_bmi(bmi)
        assert result == f"Your BMI is {bmi}, you are overweight.", "Failed to correctly interpret overweight BMI"

    def test_interpret_bmi_for_obesity_classes(self):
        bmi_class_I = 32.0
        result_class_I = interpret_bmi(bmi_class_I)
        assert result_class_I == f"Your BMI is {bmi_class_I}, you are obese (Class I).", "Failed to correctly interpret Class I obesity BMI"

        bmi_class_II = 37.0
        result_class_II = interpret_bmi(bmi_class_II)
        assert result_class_II == f"Your BMI is {bmi_class_II}, you are obese (Class II).", "Failed to correctly interpret Class II obesity BMI"

        bmi_class_III = 45.0
        result_class_III = interpret_bmi(bmi_class_III)
        assert result_class_III == f"Your BMI is {bmi_class_III}, you are obese (Class III).", "Failed to correctly interpret Class III obesity BMI"

    def test_interpret_bmi_for_invalid_input(self):
        bmi = None
        result = interpret_bmi(bmi)
        assert result == "Invalid input. Height should be greater than 0.", "Failed to correctly handle invalid input"
